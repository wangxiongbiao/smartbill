        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-all pointer-events-auto" onClick={onClose}>
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h3 className="text-2xl font-semibold text-slate-900 tracking-tight">{t.shareTitle}</h3>
                    ...
                    {activeShare ? (
                        <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                            ...
                            <div className="bg-slate-50 rounded-3xl p-6 flex flex-col items-center justify-center border border-slate-100">
                                ...
                                <span className="text-slate-400 text-xs font-semibold uppercase tracking-widest">{t.scanToShare}</span>
                            </div>

                            <div className="flex justify-between items-center pt-4 border-t border-slate-100">
                                <span className="text-xs font-semibold text-slate-400">{t.shareCreated}: {new Date(activeShare.created_at).toLocaleDateString(getLocaleForLanguage(lang))}</span>
                                <button onClick={() => handleRevoke(activeShare.id)} className="text-red-500 hover:text-red-600 text-sm font-semibold flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors"><i className="fas fa-ban"></i> {t.revoke}</button>
                            </div>
                        </div>
                    ) : (
                        <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                            <div className="bg-blue-50 rounded-3xl p-8 text-center">
                                ...
                                <h4 className="text-xl font-semibold text-slate-900 mb-2">{t.createShareLink}</h4>
                                ...
                                <div className="bg-white rounded-2xl p-4 text-left shadow-sm border border-blue-100 space-y-4 mb-6">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-slate-700 font-semibold text-sm flex items-center gap-2"><i className="fas fa-clock text-orange-400"></i> {t.expiresIn}</span>
                                        <select className="bg-slate-50 border-none text-sm font-semibold text-slate-600 rounded-lg py-1 px-3 focus:ring-0 cursor-pointer" value={expiresIn === null ? 'never' : expiresIn} onChange={e => setExpiresIn(e.target.value === 'never' ? null : Number(e.target.value))}>
                                ...
                                <button onClick={handleCreateShare} disabled={loading} className="w-full bg-blue-600 text-white py-4 rounded-xl font-semibold shadow-xl shadow-blue-200 hover:scale-[1.02] active:scale-95 transition-all text-lg">{loading ? <i className="fas fa-spinner fa-spin"></i> : t.generateLink}</button>
                            </div>
                        </div>
                    )}
                </div>
