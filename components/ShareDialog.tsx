...
const ShareDialog: React.FC<ShareDialogProps> = ({ invoice, isOpen, onClose, lang }) => {
    ...
    const handleCreateShare = async () => {
        setLoading(true);
        try {
            const options: CreateShareOptions = { allowDownload, expiresInDays: expiresIn };
    ...
    const handleRevoke = async (shareId: string) => {
        try {
            ...
            if (activeShare?.id === shareId) setActiveShare(null);
        } catch (error) {
    ...
    const shareUrl = activeShare ? `${window.location.origin}/share/${activeShare.share_token}` : '';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-all pointer-events-auto" onClick={onClose}>
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h3 className="text-2xl font-black text-slate-900 tracking-tight">{t.shareTitle}</h3>
                        <p className="text-slate-500 text-sm font-medium mt-1">{t.shareSubtitle}</p>
                    </div>
                ...
                <div className="p-8 space-y-8 min-h-[350px]">
                    {activeShare ? (
                        <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                            <div>
                                <label className="text-[10px] uppercase font-black text-slate-400 tracking-widest mb-2 block">{t.shareLink}</label>
                                <div className="flex gap-3">
                                    ...
                                    <button onClick={() => copyToClipboard(shareUrl)} className={`px-6 py-3 rounded-2xl font-bold text-white shadow-lg transition-all active:scale-95 flex items-center gap-2 ${copied ? 'bg-emerald-500 shadow-emerald-200' : 'bg-blue-600 shadow-blue-200 hover:bg-blue-700'}`}><i className={`fas ${copied ? 'fa-check' : 'fa-copy'}`}></i>{copied ? t.copied : t.copy}</button>
                                </div>
                            </div>

                            <div className="bg-slate-50 rounded-3xl p-6 flex flex-col items-center justify-center border border-slate-100">
                                <div className="bg-white p-3 rounded-xl shadow-sm mb-3"><QRCodeSVG value={shareUrl} size={160} /></div>
                                <span className="text-slate-400 text-xs font-bold uppercase tracking-widest">{t.scanToShare}</span>
                            </div>
                        ...
                        <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                            <div className="bg-blue-50 rounded-3xl p-8 text-center">
                                <div className="w-16 h-16 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4"><i className="fas fa-share-alt"></i></div>
                                <h4 className="text-xl font-bold text-slate-900 mb-2">{t.createShareLink}</h4>
                                <p className="text-slate-500 text-sm mb-6">{t.createProLinkDesc}</p>

                                <div className="bg-white rounded-2xl p-4 text-left shadow-sm border border-blue-100 space-y-4 mb-6">
                                    <div className="flex items-center justify-between">
                                        <span className="text-slate-700 font-bold text-sm flex items-center gap-2"><i className="fas fa-file-pdf text-red-400"></i> {t.allowPdfDownload}</span>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-slate-700 font-bold text-sm flex items-center gap-2"><i className="fas fa-clock text-orange-400"></i> {t.expiresIn}</span>
                                        <select className="bg-slate-50 border-none text-sm font-bold text-slate-600 rounded-lg py-1 px-3 focus:ring-0 cursor-pointer" value={expiresIn === null ? 'never' : expiresIn} onChange={e => setExpiresIn(e.target.value === 'never' ? null : Number(e.target.value))}>
                                ...
                                <button onClick={handleCreateShare} disabled={loading} className="w-full bg-blue-600 text-white py-4 rounded-xl font-black shadow-xl shadow-blue-200 hover:scale-[1.02] active:scale-95 transition-all text-lg">{loading ? <i className="fas fa-spinner fa-spin"></i> : t.generateLink}</button>
                            </div>
                        </div>
                    )}
                </div>
