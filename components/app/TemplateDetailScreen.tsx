...
export default function TemplateDetailScreen({ templateId, template, loading = false, lang, onUseTemplate, onBack, onDelete, showToast }: TemplateDetailScreenProps) {
  ...
  if (!template) return <div className="min-h-[60vh] flex items-center justify-center"><button onClick={onBack} className="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors">{copy.backToTemplates}</button></div>;
