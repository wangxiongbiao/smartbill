...
const SortableFieldItem = ({ field, onToggleVisibility, onRename, onDelete, t }: { field: PaymentInfoField; onToggleVisibility: (id: string) => void; onRename: (id: string, newLabel: string) => void; onDelete: (id: string) => void; t: any; }) => {
...
const PaymentFieldConfigurator: React.FC<PaymentFieldConfiguratorProps> = ({ fields, onChange, onClose, lang }) => {
    ...
    const [newFieldType] = useState<PaymentFieldType>('textarea');
    ...
    const toggleVisibility = (id: string) => onChange(fields.map(f => f.id === id ? { ...f, visible: !f.visible } : f));
    const renameField = (id: string, newLabel: string) => onChange(fields.map(f => f.id === id ? { ...f, label: newLabel } : f));
        ...
        <div className="absolute right-0 top-10 z-[100] w-120 bg-white rounded-xl shadow-xl border border-slate-200 p-4 animate-in fade-in zoom-in duration-200 origin-top-right">
            ...
            <div className="space-y-2 pt-3 border-t border-slate-100">
                <div className="flex gap-2">
                    <input value={newFieldName} onChange={(e) => setNewFieldName(e.target.value)} placeholder={t.newFieldName || 'New Field Name'} className="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm" onKeyDown={(e) => e.key === 'Enter' && addField()} />
                    <button onClick={addField} disabled={!newFieldName.trim()} className="px-3 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">{t.add}</button>
                </div>
            </div>
        </div>
    );
