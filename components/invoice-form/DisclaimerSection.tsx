...
export default function DisclaimerSection({ invoice, t, onChange }: DisclaimerSectionProps) {
  ...
  return (
    <div className="bg-white rounded-[24px] border border-slate-200 p-6 shadow-sm space-y-3">
      <div className="flex justify-between items-center"><div className="flex items-center gap-2"><h3 className="text-xs font-black uppercase tracking-widest text-slate-400">{copy.title}</h3><button type="button" onClick={() => onChange({ visibility: { ...invoice.visibility, disclaimer: invoice.visibility?.disclaimer === false } })} className={`text-xs ${invoice.visibility?.disclaimer !== false ? 'text-blue-600' : 'text-slate-300'}`} title={t.visibility}><i className={`fas fa-toggle-${invoice.visibility?.disclaimer !== false ? 'on' : 'off'} text-lg`}></i></button></div></div>
