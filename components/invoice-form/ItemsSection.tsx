...
export default function ItemsSection({ invoice, t, lang, columns, sortedColumns, sensors, showColumnConfig, setShowColumnConfig, onChange, addItem, removeItem, handleDragEnd, renderCell }: ItemsSectionProps) {
  ...
  return (
    <section className="space-y-4 pt-4 border-t border-slate-100">
                    ...
                    <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 select-none touch-manipulation">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{systemColumns.map(col => <div key={col.id}><label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1 block flex items-center gap-1">{col.label}{!col.visible && <i className="fas fa-eye-slash text-[8px] opacity-50" title={copy.hiddenInPreview}></i>}</label>{renderCell(item, col)}</div>)}</div>
                          {customColumns.length > 0 && <div className="pt-3 border-t border-slate-200"><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{customColumns.map(col => <div key={col.id}><label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1 block flex items-center gap-1">{col.label}{!col.visible && <i className="fas fa-eye-slash text-[8px] opacity-50" title={copy.hiddenInPreview}></i>}</label>{renderCell(item, col)}</div>)}</div></div>}
                        </div>
      ...
      <button type="button" onClick={addItem} className="w-full py-2 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 hover:border-blue-400 hover:text-blue-500 transition-all font-bold text-sm">{t.addItems}</button>
    </section>
  );
}
