  ...
  return (
    <section className="space-y-4 pt-4 border-t border-slate-100">
      <div className="flex justify-between items-center relative">
        ...
        <button onClick={() => setShowColumnConfig(!showColumnConfig)} className="text-slate-400 hover:text-blue-600 transition-colors" title={t.customizeColumns}><i className="fas fa-cog"></i></button>
                    ...
                    <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 select-none touch-manipulation">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{systemColumns.map(col => <div key={col.id}><label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1 block flex items-center gap-1">{col.label}{!col.visible && <i className="fas fa-eye-slash text-[8px] opacity-50" title="Hidden in preview"></i>}</label>{renderCell(item, col)}</div>)}</div>
                          {customColumns.length > 0 && <div className="pt-3 border-t border-slate-200"><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{customColumns.map(col => <div key={col.id}><label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1 block flex items-center gap-1">{col.label}{!col.visible && <i className="fas fa-eye-slash text-[8px] opacity-50" title="Hidden in preview"></i>}</label>{renderCell(item, col)}</div>)}</div></div>}
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); removeItem(item.id); }} className="text-slate-300 hover:text-red-500 p-2 transition-colors"><i className="fas fa-trash-alt"></i></button>
                      </div>
                    </div>
                  )}
                </SortableItem>
      ...
      <button onClick={addItem} className="w-full py-2 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 hover:border-blue-400 hover:text-blue-500 transition-all font-bold text-sm">{t.addItems}</button>
    </section>
  );
}
