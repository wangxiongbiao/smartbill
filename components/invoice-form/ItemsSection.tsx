                    ...
                    <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 select-none touch-manipulation">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{systemColumns.map(col => <div key={col.id}><label className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-1 block flex items-center gap-1">{col.label}{!col.visible && <i className="fas fa-eye-slash text-[8px] opacity-50" title={copy.hiddenInPreview}></i>}</label>{renderCell(item, col)}</div>)}</div>
                          {customColumns.length > 0 && <div className="pt-3 border-t border-slate-200"><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{customColumns.map(col => <div key={col.id}><label className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-1 block flex items-center gap-1">{col.label}{!col.visible && <i className="fas fa-eye-slash text-[8px] opacity-50" title={copy.hiddenInPreview}></i>}</label>{renderCell(item, col)}</div>)}</div></div>}
                        </div>
