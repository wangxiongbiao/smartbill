...
const TemplatesView: React.FC<TemplatesViewProps> = ({
    ...
    if (loading) {
        return <div className="min-h-[60vh] flex items-center justify-center"><div className="flex flex-col items-center gap-4"><div className="w-12 h-12 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div><p className="text-slate-400 font-medium">{t.loadingHistory || copy.loading}</p></div></div>;
    }
    ...
    if (templates.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-40 text-center">
                ...
                <button onClick={onNewDoc} className="mt-10 bg-blue-600 text-white px-8 py-4 rounded-2xl font-black shadow-[0_18px_34px_-20px_rgba(37,99,235,0.52)] hover:bg-blue-700 transition-all active:scale-95 flex items-center gap-3"><i className="fas fa-plus"></i><span>{t.newInvoice}</span></button>
            </div>
        );
    }
            ...
            <div className="px-6 py-6">
                <div className="flex flex-col gap-4 mb-4">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <button onClick={onNewDoc} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-full font-bold shadow-[0_18px_34px_-20px_rgba(37,99,235,0.52)] hover:shadow-xl transition-all flex items-center justify-center gap-2 active:scale-95 text-sm"><i className="fas fa-plus-circle"></i><span>{t.newInvoice || copy.createNew}</span></button>
                    ...
                    {currentTemplates.map((template) => (
                        <div key={template.id} className="bg-white rounded-[2rem] p-6 border border-slate-100 flex flex-col xl:flex-row items-center gap-6 xl:gap-0 hover:shadow-lg transition-all group cursor-pointer" onClick={() => onViewDetail(template)}>
                            ...
                            <div className="flex items-center gap-2 w-full xl:w-[15%] justify-end" onClick={(e) => e.stopPropagation()}>
                                <button onClick={(e) => { e.stopPropagation(); handleUseTemplate(template); }} className="w-8 h-8 rounded-lg bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white flex items-center justify-center transition-colors" title={t.useTemplate}><i className="fas fa-plus text-xs"></i></button>
                ...
                {filteredTemplates.length > 0 && (
                    <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-6">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-4 py-2 rounded-xl border border-slate-200 disabled:opacity-50">{copy.prev}</button>
                            <span className="text-sm font-bold text-slate-500">{currentPage} / {Math.max(totalPages, 1)}</span>
                            <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages || totalPages === 0} className="px-4 py-2 rounded-xl border border-slate-200 disabled:opacity-50">{copy.next}</button>
                        </div>
                    </div>
                )}
            </div>
