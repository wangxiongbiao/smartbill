                        ...
                        <div className="space-y-8">
                            <div className="space-y-3">
                                ...
                                <textarea
                                    ...
                                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl h-32 text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-medium"
                                />
                            </div>
...
let startIdx = fileStr.indexOf('</DndContext>');
if (startIdx === -1) {
    console.error("Could not find </DndContext>!");
    process.exit(1);
}

// Ensure the part up to `</DndContext>` is kept, then append `fixPart`, then append the rest starting from `<ImagePickerDialog`
let newStr = fileStr.substring(0, startIdx) + fixPart;

let imagePickerIdx = fileStr.indexOf('isOpen={showLogoPickerDialog}', startIdx);
if (imagePickerIdx === -1) {
    // try to find just showLogoPickerDialog
    imagePickerIdx = fileStr.indexOf('showLogoPickerDialog');
    if (imagePickerIdx !== -1) {
        // backtrack to <ImagePickerDialog
        let dialogIdx = fileStr.lastIndexOf('<ImagePickerDialog', imagePickerIdx);
        if (dialogIdx !== -1) {
           imagePickerIdx = dialogIdx + '<ImagePickerDialog'.length;
        }
    }
}
...
if (imagePickerIdx !== -1) {
    // Append the rest
    // since we appended <ImagePickerDialog in fixPart, we just append from `isOpen={...`
    newStr += `
                isOpen={showLogoPickerDialog}
` + fileStr.substring(fileStr.indexOf('isOpen={showLogoPickerDialog}', startIdx) + 'isOpen={showLogoPickerDialog}'.length);
}
